...
public class NPCScript : MonoBehaviour {
	...
	NPCScript[] linkedNPCs;

	void LinkNPCs()
	{
		if(linkedNPCs.Length == 1) return;
		TBScript.usedAnimators = new Animator[(linkedNPCs.Length > 4 ? 4 : linkedNPCs.Length)+1];
		for(int i = 1; i < (linkedNPCs.Length > 3 ? 3 : linkedNPCs.Length); i++)
		{
			Transform obj = linkedNPCs[i].transform;
			TBScript.usedAnimators[i+1] = obj.GetComponent<Animator>();
			TBScript.poor4Voices[i-1] = linkedNPCs[i].npcVoice;
	...
	void Start () {
		if(player==null)
		{
			...
			if(!isBlock && transform.childCount != 0) bubble = transform.GetChild(0);

			linkedNPCs = transform.GetComponentsInChildren<NPCScript>();

			foreach (NPCScript n in linkedNPCs)
			{
				if(n == this) continue;
				n.transform.SetParent(transform.parent);
				Destroy(n.transform.GetChild(0).gameObject);
			}
			...
			if(!isBlock && bubble != null) bubble.gameObject.SetActive(false);
			else canTalk=true;
			...
			if(inHub)
			{
				visible = true;
				render.enabled = true;
				if(!talkedTo&&!isBlock && bubble != null)
	...
	void Update () {
		if(visible)
		{
			if(player.position.x>=transform.position.x&&facingLeft&&turnsToPlayer)
			{
				facingLeft = false;
				transform.localScale = new Vector3(-1,1,1);
			}
			else if(player.position.x<transform.position.x&&!facingLeft&&turnsToPlayer)
			{
				facingLeft = true;
				transform.localScale = new Vector3(1,1,1);
			}
			//canTalk, !isblock, Time.timeScale!=0
			if((canTalk&&!isBlock&&Time.timeScale!=0&&bubble!=null)
